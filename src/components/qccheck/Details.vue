<template>
<div class="components-container quality" id="components-container">
	...
	<split-pane v-on:resize="resize" :min-percent='35' :default-percent='0' split="vertical">
      ...
      <div class="right-box" ref="paneRbox">
          ...
          <el-tabs v-model="activeName" type="card" style="padding-left:10px;">
            ...
            <el-tab-pane label="质检意见" name="six">
            	<div class="right-container">
            	<qc-remark :apply-id="applyId"></qc-remark>
            	</div>
            </el-tab-pane>
          </el-tabs>
        </div>
...
import QcRemark from '@/common/display/QcRemark.vue'
import {
	imgType,
	getImgTypeLabel
...
export default {
	...
	methods: {
		resize(percent) {
			// var pwidth = document.getElementById("components-container").offsetWidth;
			// var obj = document.getElementsByClassName("left-container")[0];
			// var leftW = obj.offsetWidth;
			// var mask = document.getElementsByClassName("viewer-container")[0];
			// var canvas = document.getElementById("viewer-canvas");
			// //console.info(pwidth * percent / 100)
			// mask.style.width = (pwidth * percent / 100) + "px"
			// canvas.style.width = (pwidth * percent / 100) + "px"
			// obj.style.width = (pwidth * percent / 100) + "px"
		},
		toggleIcon() {
			if (this.init == true) {
