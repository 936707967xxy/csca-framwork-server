<!--
	Created by yangfei on 2018/4/20.
    信息变更历史记录
-->
<template>
  <div id="normal-type-box" class="wrap-box money-daihou-style">
		<div class="crumbs hander-class">
			<el-breadcrumb separator="/" separator-class="el-icon-arrow-right">
				...
				<el-breadcrumb-item>信息变更一级审批</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
        ...
        <el-form-item label="申请类型:" prop="applyType" >
          <el-select v-model="listQuery.applyType" placeholder="请选择" size="small">
            ...
            <el-option label="银行卡号" value="bankChange"></el-option>
            <el-option label="手机号码 " value="telChange" ></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-row>
          <el-col class="form-r-btn">
            <el-form-item>
              <el-button type="warning" @click="applyDialog" size="small">审批详情</el-button>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" :data="tableData" :stripe="true" border style="width:100%" cell-class-name="cell-class" @selection-change="selchage" height="570" size="medium">
        ...
        <el-table-column prop="applyType" label="申请类型" min-width="120" align="center" fixed>
          <template  slot-scope="props">
              <span v-show="props.row.applyType=='bankChange'">银行卡号</span>
              <span v-show="props.row.applyType=='telChange'">手机号码</span>
          </template>
        </el-table-column>
...
import messageUpdate from  "./msgUpdateHistoryDetails.vue";
...
export default {
  ...
  methods: {
    getPager() {
      fetchData("/after/messageupdate/changeinfoupdateHistory.do",this.listQuery).then(response => {
    ...
    selchage(selection) {
  	...
  	applyDialog() {
          ...
          this.messageUpdateForm.applyId = this.selectList[0].applyId;
