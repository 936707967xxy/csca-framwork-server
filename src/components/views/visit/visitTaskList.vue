  ...
  <div id="normal-type-box" class="wrap-box money-daihou-style">
		<div class="crumbs hander-class">
			<el-breadcrumb separator="/" separator-class="el-icon-arrow-right">
				...
				<el-breadcrumb-item>外访分配</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
        ...
        <div>
          <el-form-item class="form-r-btn">
            <el-button type="primary" @click="monthDeduct('0')" size="small">外访分配</el-button>
    ...
    <el-dialog title="职员信息列表" :visible.sync="dialogDetailTable" width="60%" center :close-on-click-modal='false'>
      <el-table :data="empList" :stripe="true" border style="width:100%" cell-class-name="cell-class" @select="selchage1" height="450" >
        <el-table-column type="selection" width="46" center></el-table-column>
				<el-table-column prop="empId" label="部门编号" min-width="300" align="center" fixed></el-table-column>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="cancelFun" size="small">取消分派</el-button>
        <el-button type="primary" @click="visitTaskFun" size="small">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    empListFun(state) {
      ...
      createDataJson( "/after/visit/queryVisitTaskDetailsService.do",data).then(response => {
    ...
    visitTaskFun() {
      if (this.selectList1.length < 1) {
        ...
        return;
      }else if (this.selectList1.length > 1) {
        this.$message({
          message: "警告,只允许单笔划扣!",
          type: "warning",
          duration:5000
        ...
        let data = {
          loanIdList:this.loanIdList,
          empWorkNum:this.selectList1[0].empWorkNum,
          empName:this.selectList1[0].empName,
        };
        this.submitFun(data);
      }
    ...
    submitFun(data) {
      createDataJson( "/after/visit/visitTaskService.do",data).then(response => {
