  ...
  <div id="normal-type-box" class="wrap-box money-daihou-style">
		<div class="crumbs hander-class">
			<el-breadcrumb separator="/" separator-class="el-icon-arrow-right">
				...
				<el-breadcrumb-item>外访申请记录</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
        ...
        <el-form-item label="申请类型:" prop="applyType" >
          <el-select v-model="listQuery.applyType" placeholder="请选择" size="small">
            ...
            <el-option label="外访申请" value="outBound"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-row>
          <el-col class="form-r-btn">
            <el-form-item>
              <el-button type="warning" @click="applyDialog()" size="small">外访审批详情</el-button>
    ...
    <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" :data="tableData"
        ...
        <el-table-column prop="applyType" label="申请类型" min-width="120" align="center" fixed>
          <template  slot-scope="props">
              <span v-show="props.row.applyType=='outBound'">外访申请</span>
          </template>
        </el-table-column>
    ...
    <outbound-apply ref="outboundApplyeDialog" @init="getPager" :outboundApplyParams="outboundApplyParams"></outbound-apply>
...
import outboundApply from "./outboundApplyDetails2.vue";
...
export default {
  ...
  methods: {
    getPager() {
      fetchData("/after/outbound/outboundHistory.do",this.listQuery).then(response => {
    ...
    selchage(selection) {
  	...
  	applyDialog() {
          ...
          this.outboundApplyParams.loanId = this.selectList[0].loanId;
          this.outboundApplyParams.applyId = this.selectList[0].applyId;
          this.$refs.outboundApplyeDialog.show();
				}
      },
